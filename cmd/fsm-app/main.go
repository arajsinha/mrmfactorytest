...
// RestartExecution contains the corrected logic for finding tasks.
func RestartExecution(sid string, eid string, etcd *clientv3.Client) {
	logger := slog.New(slog.NewJSONHandler(os.Stdout, nil))
	logger = logger.With("sid", sid, "eid", eid)
	logger.Info("Starting restart process for execution.")

	ctx := context.Background()
	execStore := store.NewExecutionStore(etcd, logger, sid, eid)
	...
	if !foundTasks {
		logger.Error("Restart failed: could not find any tasks for target state in the current config.", "state", header.TargetState)
		return
	}
	// --- END OF FIX ---
	
	runner := taskrunner.NewRunner(logger, 10, "RESTART")
	runner.ExecuteActions(ctx, tasksToRun, execStore)
	logger.Info("Restart execution has been handed off to the runner.")
}


// ... (The rest of the file is unchanged) ...
func Execute(command string, etcd *clientv3.Client) {
	configPath := "fsm-config.yaml"
	numWorkers := 10
	logger := slog.New(slog.NewJSONHandler(os.Stdout, nil))
	baseStore := store.NewExecutionStore(etcd, logger, scenarioID, "")

	cfg, err := bootstrapFSMConfig(context.Background(), logger, baseStore, configPath)
	if err != nil {
		logger.Error("FATAL: Could not bootstrap FSM configuration.", "error", err)
		os.Exit(1)
	}
	logger.Info("FSM configuration is synchronized with etcd.")
	...
	runner := taskrunner.NewRunner(logger, numWorkers, command)
	fsmMachine := fsm.NewMachine(ctx, cfg, runner, logger, baseStore)
	fsmMachine.Run(ctx)
...
func bootstrapFSMConfig(ctx context.Context, logger *slog.Logger, s *store.ExecutionStore, configPath string) (*config.Config, error) {
	...
	logger.Info("Successfully wrote local config to etcd.")
